            ...
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
				   ...
				   <el-form-item>
					...
					<el-form-item>
                        <el-button v-if="isAuth('haixian','点餐') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1" type="success" icon="el-icon-plus" @click="deleteHandler()">{{ contents.btnAdAllFont == 1?'确定订单':'' }}</el-button>
                        <el-button v-if="isAuth('haixian','点餐') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2" type="success" @click="deleteHandler()">{{ contents.btnAdAllFont == 1?'确定订单':'' }}<i class="el-icon-plus el-icon--right" /></el-button>
                        <el-button v-if="isAuth('haixian','点餐') && contents.btnAdAllIcon == 0" type="success" @click="deleteHandler()">{{ contents.btnAdAllFont == 1?'确定订单':'' }}</el-button>
                    </el-form-item>
                </el-row>
            </el-form>
                    ...
                    <el-table-column label="索引" v-if="contents.tableIndex" type="index" width="50" />
                          ...
                          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="imgPhoto"
                             ...
                             <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                               ...
                                               label="选择数量">
                                 <template slot-scope="scope">
                                    <input type="number" :id="scope.row.hxValue"  value="0" :min="1" :max="5" label="描述文字" style="width: 150px;height: 50px;font-size: 25px;"/>
                                 </template>
                             </el-table-column>
                </el-table>
                <el-pagination
    ...
    export default {
        ...
        methods: {
			...
			for(var i= 0 ;i < ids.length;i++){
				 paramArray[ids[i].hxValue] = $("[id="+ids[i].hxValue+"]").val();
			}
                this.$confirm(`确定进行点餐操作?`, '提示', {
